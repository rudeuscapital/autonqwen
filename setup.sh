...
configure() {
  ...
  echo -e "  ${CYN}1${RST}) qwen3:1.7b     ~1GB  — Fast, lightweight (default)"
  echo -e "  ${CYN}2${RST}) qwen3.5        ~4GB  — Great for most tasks"
  echo -e "  ${CYN}3${RST}) qwen3.5:7b     ~5GB  — Slightly larger variant"
  echo -e "  ${CYN}4${RST}) qwen3.5:14b    ~9GB  — Better quality (needs 16GB RAM)"
  echo -e "  ${CYN}5${RST}) qwen3:8b       ~5GB  — Qwen3 8B, excellent tool calling"
  echo -e "  ${CYN}6${RST}) llama3.2:3b    ~2GB  — Ultra-light (low RAM servers)"
  echo -e "  ${CYN}7${RST}) Skip           — Pull model manually later"
  read -rp "  Choice [1]: " MODEL_CHOICE
  MODEL_CHOICE="${MODEL_CHOICE:-1}"

  case "$MODEL_CHOICE" in
    1) OLLAMA_MODEL="qwen3:1.7b" ;;
    2) OLLAMA_MODEL="qwen3.5"    ;;
    3) OLLAMA_MODEL="qwen3.5:7b" ;;
    4) OLLAMA_MODEL="qwen3.5:14b";;
    5) OLLAMA_MODEL="qwen3:8b"   ;;
    6) OLLAMA_MODEL="llama3.2:3b";;
    7) OLLAMA_MODEL=""           ;;
...
DEFAULT_MODEL=${OLLAMA_MODEL:-qwen3:1.7b}
...
print_summary() {
  ...
  echo -e "  ${CYN}aq pull qwen3:1.7b${RST}  — pull a different model"
  ...
  if [[ -n "$OLLAMA_MODEL" ]]; then
    ...
    echo -e "     ${CYN}aq pull qwen3:1.7b${RST}   # or any other model"
    echo
  fi
